...
import { creerEncaissement, listerClients, listerCommerciaux } from '../api/ressources'
...
export default function Encaissement() {
  ...
  async function handleSubmit(event) {
    ...
    setEnCours(true)
    try {
      await creerEncaissement({
        uuid: crypto.randomUUID(),
        client: Number(clientId),
        montant,
        moyen_paiement: moyenPaiement,
        collecte_par: collectePar ? Number(collectePar) : undefined,
        date_encaissement: new Date().toISOString(),
      })
      setSucces('Encaissement enregistré.')
      setMontant('')
    } catch {
      setErreur("Impossible d'enregistrer cet encaissement.")
    } finally {
      setEnCours(false)
    }
  }
